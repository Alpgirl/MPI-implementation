...
int main() {
    int i, j, N_x = 100, N_y = 100, time = 461, t, t_c = 460, z;
    cout << setprecision(10);

    double L_x = 10.0, L_y = 20.0, T_1 = 1000.0, T_2 = 300.0, ai, bi, ci, fi;
    double h_x, h_y, tau, n, h = 7.0, coef_time, a = 1.0;//pow(2,-0.5);
    double test, test1, test2;
    double ***T = new double ** [(N_x)];
    double **alpha = new double * [2];
    double **T_exact = new double * [N_x];

    ofstream on_exact_1("file_analytical_compare_mpi.dat");
    ...
    h_x = L_x/(N_x+1);
    h_y = L_y/(N_y+1);
    tau = 1.0/4*pow(min(h_x,h_y)/a,2); // оптимальное время
        ...
        for (i = 0; i < N_x; i++){
            for (j = 0; j < N_y; j++) {
                ...
                //else {
                    ...
                    T[i][j][t+1] = tau * pow(a,2) * ((alpha[0][0] + alpha[0][1])/pow(h_x,2) + (alpha[1][0] + alpha[1][1])/pow(h_y,2)) + T[i][j][t];
                //}
            }
        }
    }
    ...
    for (j = 0; j < N_y; j++){
        for (i = 0; i < N_x; i++){
            //on_exact_1 << endl;
            on_exact_1 << /*i << " " << j << " " << T[i][j][t_c] << " ";  T_exact[i][j] << " "; <<*/  T[i][j][t_c] << " ";
        }
        on_exact_1 << endl;
    }
    delete[] T;
